//! Batched GPU operations (use CommandBatch instead of sync per-op).

use super::dispatch::dispatch_compute_batched;
use super::ops::BW_DEFAULT_HEADROOM;
...
pub fn apply_inversion_batched(
...
pub fn apply_shadow_lift_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
...
pub fn apply_highlight_compression_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
    threshold: f32,
...
pub fn apply_base_offsets_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
    offsets: [f32; 3],
...
pub fn apply_color_matrix_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
    matrix: &[[f32; 3]; 3],
...
pub fn apply_tone_curve_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
    options: &ConvertOptions,
...
pub fn apply_hsl_adjustments_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
...
pub fn clamp_working_range_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
    pixel_count: u32,
...
pub fn apply_cb_inversion_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
...
pub fn apply_cb_layers_batched(
    batch: &mut CommandBatch,
    ctx: &GpuContext,
    image: &GpuImage,
