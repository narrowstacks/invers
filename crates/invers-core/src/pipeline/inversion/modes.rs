//! Inversion mode implementations
//!
//! Contains the main `invert_negative` function and implementations for:
//! - Linear inversion
//! - Logarithmic (density-based) inversion
//! - DivideBlend inversion (Photoshop-style)
//! - MaskAware inversion (orange mask compensation)
...
pub fn invert_negative(
    ...
    match options.inversion_mode {
        InversionMode::Linear => {
            apply_linear_inversion(data, base_r, base_g, base_b);
        }
        InversionMode::Logarithmic => {
            apply_logarithmic_inversion(data, base_r, base_g, base_b);
        }
        InversionMode::DivideBlend => {
            apply_divide_blend_inversion(data, base_r, base_g, base_b);
        }
        InversionMode::MaskAware => {
            apply_mask_aware_inversion(data, base, base_r, base_g, base_b, options);
        }
        InversionMode::BlackAndWhite => {
            apply_bw_inversion(data, base_r, base_g, base_b, options);
        }
    }

    // Apply shadow lift based on mode
    apply_shadow_lift(data, options);
...
fn apply_linear_inversion(data: &mut [f32], base_r: f32, base_g: f32, base_b: f32) {
...
///
/// OPTIMIZATION: Use natural log/exp which are faster than log10/powf(10)
/// Math equivalence: 10^(log10(a) - log10(b)) = exp(ln(a) - ln(b)) = a/b
/// But we keep the log-space calculation for numerical stability at extremes
fn apply_logarithmic_inversion(data: &mut [f32], base_r: f32, base_g: f32, base_b: f32) {
    let ln_base_r = base_r.ln();
    let ln_base_g = base_g.ln();
    let ln_base_b = base_b.ln();

    // Pre-compute clamp threshold in log space: ln(10.0)
    const LN_MAX: f32 = std::f32::consts::LN_10;

    for pixel in data.chunks_exact_mut(3) {
        // Use fast_max to avoid branch misprediction
        let neg_r = pixel[0].max(0.0001);
        let neg_g = pixel[1].max(0.0001);
        let neg_b = pixel[2].max(0.0001);

        // ln(base) - ln(neg) = ln(base/neg), then exp() to get result
        // Clamp in log space before exp() to avoid overflow
        pixel[0] = (ln_base_r - neg_r.ln()).min(LN_MAX).exp();
        pixel[1] = (ln_base_g - neg_g.ln()).min(LN_MAX).exp();
        pixel[2] = (ln_base_b - neg_b.ln()).min(LN_MAX).exp();
    }
}

/// Divide blend inversion (Photoshop-style workflow)
///
...
fn apply_divide_blend_inversion(data: &mut [f32], base_r: f32, base_g: f32, base_b: f32) {
...
/// Apply shadow lift based on mode
fn apply_shadow_lift(data: &mut [f32], options: &ConvertOptions) {
