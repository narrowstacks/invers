//! Grid search functions for parameter optimization
//!
//! Contains functions for exhaustive and adaptive parameter grid searches.

use crate::models::{BaseSamplingMode, InversionMode, ShadowLiftMode};
use crate::testing::runners::{run_parameter_test, run_parameter_test_preloaded};
use crate::testing::types::{ParameterGrid, ParameterTest, PreloadedTestContext, TestResult};
use rayon::prelude::*;
use std::path::Path;
    ...
    for iteration in 1..=max_iterations {
        ...
        if let Some(best) = iteration_results.first() {
            if best.overall_score < best_score {
                ...
                best_params = best.params.clone();
                eprintln!(
                    "[ADAPTIVE SEARCH] Improved! New best: {:.4} (delta {:.4})",
...
pub fn print_test_result(result: &TestResult, rank: usize) {
    ...
    println!(
        "  Exposure Match:     {}",
        if exposure_ok { "GOOD" } else { "NEEDS WORK" }
    );
    println!(
        "  Color Balance:      {}",
        if color_ok { "GOOD" } else { "NEEDS WORK" }
    );
    println!(
        "  Overall Accuracy:   {}",
